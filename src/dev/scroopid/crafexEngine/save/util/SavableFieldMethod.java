...
public class SavableFieldMethod {
	 ...
	 * 
	 * It has to return a {@link String[]} and have these parameters in this order {@link Object} target, {@link Field}
	 * field, {@link int} callsDeep
	 * 
	 * @param method
	 *        The method to check
	 ...
	 * 
	 * @param method
	 *        The method to base around
	 * @throws SaveException
	 *         If the method is null or invalid
	 */
	...
	public List<String> save(Object target, Field field, int callsDeep) {
		...
		try {
			result = (String[]) this.saveMethod.invoke(null, target, field, callsDeep);
		} catch (IllegalArgumentException e) {

			// This should not happen... haha... sEx
			SaveException sEx = new SaveException("Illegal Arguements");
			LOGGER.error(String.format("Method %s has illegal arguements?", this.methodName), sEx);
			throw sEx;
			...
			LOGGER.error(String.format("Method %s cannot be accessed", this.methodName), sEx);
			throw sEx;

		} catch (InvocationTargetException e) {

			// The method cannot be invoked... sEx
			SaveException sEx = new SaveException("Cannot invoke the method: " + this.methodName);
			LOGGER.error(String.format("Method %s cannot be invoked", this.methodName), sEx);
			throw sEx;
